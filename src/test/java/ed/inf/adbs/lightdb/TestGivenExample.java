...
public class TestGivenExample {
    ...
    public boolean compare(String num) {
        String[] args = new String[]{"samples/db", "samples/input/query" + num + ".sql", "output/query" + num + ".csv"};
        LightDB.main(args);
        String expected = "";
        try {
        ...
        String my = "";
        try {
            File file = new File("output/query" + num + ".csv");
            Scanner scanner = new Scanner(file);
            while (scanner.hasNext()) {
                my = my + scanner.next() + "\n";
            }
        ...
        File myObj = new File("output/query" + num + ".csv");
        if (myObj.delete()) {
